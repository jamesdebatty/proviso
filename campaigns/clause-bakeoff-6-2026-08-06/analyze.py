#!/usr/bin/env python3
"""Blindly grade the two-arm clause bakeoff 6 with two judges (Sonnet 5 + Codex)."""
...
def judge_sonnet(judge_dir: Path, task: dict, repetition: int, prompt: str, expected: dict) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="clause-bakeoff6-judge-") as cwd:
        proc = subprocess.run(
...
def judge_codex(judge_dir: Path, task: dict, repetition: int, prompt: str, expected: dict) -> dict:
    ...
    if cached:
        return cached
    started = time.monotonic()
    with tempfile.TemporaryDirectory(prefix="clause-bakeoff6-codex-") as temporary:
