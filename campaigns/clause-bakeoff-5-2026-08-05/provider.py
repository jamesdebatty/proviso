"""Promptfoo provider for isolated Claude Code clause variants (bakeoff 5)."""
...
VARIANT_FILES = {
    "control": None,
    "subtractive": "variants/subtractive.md",
    "guarded-subtractive": "variants/guarded-subtractive.md",
...
def artifact_dir(task_id: str, repetition: int, arm: str) -> Path:
    ...
    root = Path(os.environ["BAKEOFF5_OUT_DIR"]).resolve() / "responses"
...
def call_api(prompt: str, options: dict, context: dict) -> dict:
    ...
    expected_cli = os.environ.get("BAKEOFF5_CLAUDE_VERSION")
    if not expected_cli:
        return {"error": "BAKEOFF5_CLAUDE_VERSION is required"}
    try:
        ...
        with tempfile.TemporaryDirectory(
            ...
            with tempfile.TemporaryDirectory(prefix="opus5-clause-bakeoff5-") as temporary:
