#!/usr/bin/env python3
"""Blindly grade the four-arm clause bakeoff 5 with two judges (Sonnet 5 + Codex)."""
...
ARMS = ("control", "subtractive", "guarded-subtractive", "budgeted-recipe")
LABELS = ("A", "B", "C", "D")
...
def judge_sonnet(judge_dir: Path, task: dict, repetition: int, prompt: str, expected: dict) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="clause-bakeoff5-judge-") as cwd:
        proc = subprocess.run(
...
def judge_codex(judge_dir: Path, task: dict, repetition: int, prompt: str, expected: dict) -> dict:
    ...
    if cached:
        return cached
    started = time.monotonic()
    with tempfile.TemporaryDirectory(prefix="clause-bakeoff5-codex-") as temporary:
