"""Promptfoo provider for isolated Claude Code clause variants (bakeoff 7)."""
...
VARIANT_FILES = {
    "v2-only": "variants/guarded-subtractive-v2.md",
    "v2-plus-numbers": "variants/v2-plus-numbers.md",
}
...
def artifact_dir(task_id: str, repetition: int, arm: str) -> Path:
    ...
    root = Path(os.environ["BAKEOFF7_OUT_DIR"]).resolve() / "responses"
...
def call_api(prompt: str, options: dict, context: dict) -> dict:
    ...
    expected_cli = os.environ.get("BAKEOFF7_CLAUDE_VERSION")
    if not expected_cli:
        return {"error": "BAKEOFF7_CLAUDE_VERSION is required"}
    try:
        ...
        with tempfile.TemporaryDirectory(
            ...
            with tempfile.TemporaryDirectory(prefix="opus5-clause-bakeoff7-") as temporary:
