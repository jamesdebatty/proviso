"""Promptfoo provider for isolated Claude Code clause variants (bakeoff 4)."""
...
VARIANT_FILES = {
    "control": None,
    "positive-recipe": "variants/positive-recipe.md",
    "budgeted-recipe": "variants/budgeted-recipe.md",
    "structured-recipe": "variants/structured-recipe.md",
}
...
def artifact_dir(task_id: str, repetition: int, arm: str) -> Path:
    ...
    root = Path(os.environ["BAKEOFF4_OUT_DIR"]).resolve() / "responses"
...
def call_api(prompt: str, options: dict, context: dict) -> dict:
    ...
    expected_cli = os.environ.get("BAKEOFF4_CLAUDE_VERSION")
    if not expected_cli:
        return {"error": "BAKEOFF4_CLAUDE_VERSION is required"}
    try:
        ...
        with tempfile.TemporaryDirectory(
            ...
            with tempfile.TemporaryDirectory(prefix="opus5-clause-bakeoff4-") as temporary:
