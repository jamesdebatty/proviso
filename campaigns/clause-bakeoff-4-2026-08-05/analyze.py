#!/usr/bin/env python3
"""Blindly grade the four-arm clause bakeoff 4 with two judges (Sonnet 5 + Codex)."""
...
ARMS = ("control", "positive-recipe", "budgeted-recipe", "structured-recipe")
LABELS = ("A", "B", "C", "D")
...
def judge_sonnet(judge_dir: Path, task: dict, repetition: int, prompt: str, expected: dict) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="clause-bakeoff4-judge-") as cwd:
        proc = subprocess.run(
...
def judge_codex(judge_dir: Path, task: dict, repetition: int, prompt: str, expected: dict) -> dict:
    ...
    if cached:
        return cached
    started = time.monotonic()
    with tempfile.TemporaryDirectory(prefix="clause-bakeoff4-codex-") as temporary:
...
def main() -> int:
    ...
    print(f"{len(work)} four-answer sets; up to {2 * len(work)} judge calls (sonnet + codex)")
    if not args.execute:
        return 0
    stalls = {}
    for task in tasks():
