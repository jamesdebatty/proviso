"""Promptfoo provider for isolated Claude Code clause variants (bakeoff 8)."""
...
VARIANT_FILES = {
    "control": None,
    "composite": "variants/composite.md",
    "composite-plus-vocabulary": "variants/composite-plus-vocabulary.md",
}
...
def artifact_dir(task_id: str, repetition: int, arm: str) -> Path:
    ...
    root = Path(os.environ["BAKEOFF8_OUT_DIR"]).resolve() / "responses"
...
def call_api(prompt: str, options: dict, context: dict) -> dict:
    ...
    expected_cli = os.environ.get("BAKEOFF8_CLAUDE_VERSION")
    if not expected_cli:
        return {"error": "BAKEOFF8_CLAUDE_VERSION is required"}
    try:
        ...
        with tempfile.TemporaryDirectory(
            ...
            with tempfile.TemporaryDirectory(prefix="opus5-clause-bakeoff8-") as temporary:
