#!/usr/bin/env python3
"""Blindly grade the three-arm clause bakeoff with two judges (Sonnet 5 + Codex)."""
...
ARMS = ("control", "positive-recipe", "complete-recipe")
LABELS = ("A", "B", "C")
...
def preflight(text: str) -> str:
    """Classify a response as answered or stalled (frozen rule)."""
...
def judge_sonnet(judge_dir: Path, task: dict, repetition: int, prompt: str, expected: dict) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="clause-bakeoff3-judge-") as cwd:
        proc = subprocess.run(
...
def judge_codex(judge_dir: Path, task: dict, repetition: int, prompt: str, expected: dict) -> dict:
    ...
    if cached:
        return cached
    started = time.monotonic()
    with tempfile.TemporaryDirectory(prefix="clause-bakeoff3-codex-") as temporary:
...
def summarize(
    ...
    comparisons = {
        judge: {arm: defaultdict(int) for arm in ARMS if arm != "control"} for judge in JUDGES
    }
    if not judgments:
        ...
        for judge, item in pair.items():
            ...
            for arm in comparisons[judge]:
                result = pairwise_result(
                    item["judgment"]["pairwise"], inverse[arm], inverse["control"]
                )
    ...
    arms = {}
    control_nuance = {
        judge: statistics.mean(scores[judge]["control"]["nuance_and_safety"])
        for judge in JUDGES
    }
    control_median_words = statistics.median(words["control"]) if words["control"] else None
    ...
    for arm in ARMS:
        ...
        if arm != "control":
            ...
            gates = {
                ...
                "omissions_not_above_control": all(
                    omissions[judge][arm] <= omissions[judge]["control"] for judge in JUDGES
                ),
                "nuance_within_margin": all(
                    per_judge[judge]["nuance_delta"] >= -0.25 for judge in JUDGES
                ),
                "median_words_not_above_control": (
                    result["median_words"] is not None
                    and control_median_words is not None
                    and result["median_words"] <= control_median_words
                ),
                "stalls_not_above_control": stall_counts[arm] <= stall_counts["control"],
            }
            result["gates"] = gates
    ...
    needs_review = any(
        arms[arm]["unresolved_disputed_errors"] for arm in ARMS if arm != "control"
    )
