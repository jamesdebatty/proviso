"""Promptfoo provider for isolated Claude Code clause variants (bakeoff 3)."""
...
VARIANT_FILES = {
    ...
    "complete-recipe": "variants/complete-recipe.md",
}
...
def artifact_dir(task_id: str, repetition: int, arm: str) -> Path:
    ...
    root = Path(os.environ["BAKEOFF3_OUT_DIR"]).resolve() / "responses"
...
def call_api(prompt: str, options: dict, context: dict) -> dict:
    ...
    expected_cli = os.environ.get("BAKEOFF3_CLAUDE_VERSION")
    if not expected_cli:
        return {"error": "BAKEOFF3_CLAUDE_VERSION is required"}
    try:
        ...
        with tempfile.TemporaryDirectory(
            ...
            with tempfile.TemporaryDirectory(prefix="opus5-clause-bakeoff3-") as temporary:
