"""Promptfoo provider for isolated Claude Code clause variants."""
...
VARIANT_FILES = {
    ...
    "adaptive-recipe": "variants/adaptive-recipe.md",
}
...
def artifact_dir(task_id: str, repetition: int, arm: str) -> Path:
    ...
    root = Path(os.environ["BAKEOFF_OUT_DIR"]).resolve() / "responses"
...
def call_api(prompt: str, options: dict, context: dict) -> dict:
    ...
    expected_cli = os.environ.get("BAKEOFF_CLAUDE_VERSION")
    if not expected_cli:
        return {"error": "BAKEOFF_CLAUDE_VERSION is required"}
    try:
        ...
        with tempfile.TemporaryDirectory(
            ...
            with tempfile.TemporaryDirectory(prefix="opus5-clause-bakeoff-") as temporary:
