...
CAMPAIGN = ROOT / "campaigns" / "clause-bakeoff-5-2026-08-05"
BAKEOFF4 = ROOT / "campaigns" / "clause-bakeoff-4-2026-08-05"
...
provider = load_module("bakeoff5_provider", CAMPAIGN / "provider.py")
tests_module = load_module("bakeoff5_tests", CAMPAIGN / "tests.py")
sys.modules["provider"] = provider
analyze = load_module("bakeoff5_analyze", CAMPAIGN / "analyze.py")
...
class Bakeoff4ArtifactTests(unittest.TestCase):
    def test_budgeted_recipe_is_byte_identical_to_bakeoff_4(self):
        self.assertEqual(
            (BAKEOFF4 / "variants" / "budgeted-recipe.md").read_bytes(),
            (CAMPAIGN / "variants" / "budgeted-recipe.md").read_bytes(),
        )
    ...
    def test_battery_has_five_new_complete_tasks_with_no_tools_line(self):
        ...
        prior_prompts = {
            ...
            for source in (BAKEOFF2, BAKEOFF3, BAKEOFF4)
    ...
    def test_provider_arms_match_variant_files(self):
        ...
        self.assertEqual(
            {"control", "subtractive", "guarded-subtractive", "budgeted-recipe"},
            provider.ARMS,
        )
...
class Bakeoff4RuleTests(unittest.TestCase):
    ...
    def test_judge_prompt_contains_four_answers(self):
        ...
        for arm in ("subtractive", "guarded-subtractive", "budgeted-recipe"):
